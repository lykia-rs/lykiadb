...
mod tests {
    ...
    fn test_modulo_basic() {
        ...
        let result = nt_modulo(&mut interpreter, &Span::default(), &[RV::Num(10.0), RV::Num(3.0)]);
    ...
    fn test_modulo_zero_dividend() {
        let mut interpreter = create_test_interpreter(Some(alloc_shared(Output::new())));
        let result = nt_modulo(&mut interpreter, &Span::default(), &[RV::Num(0.0), RV::Num(5.0)]);
    ...
    fn test_modulo_zero_divisor() {
        let mut interpreter = create_test_interpreter(Some(alloc_shared(Output::new())));
        let result = nt_modulo(&mut interpreter, &Span::default(), &[RV::Num(10.0), RV::Num(0.0)]);
    ...
    fn test_modulo_invalid_type() {
        let mut interpreter = create_test_interpreter(Some(alloc_shared(Output::new())));
        let result = nt_modulo(&mut interpreter, &Span::default(), &[RV::Str(Arc::new("foo".to_string())), RV::Num(3.0)]);
        assert!(result.is_err());
    }
}