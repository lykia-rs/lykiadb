...
use super::RV;
#[derive(Debug)]
pub struct EnvironmentFrame<'session> {
    map: RwLock<FxHashMap<Symbol, RV<'session>>>,
    pub parent: Option<Arc<EnvironmentFrame<'session>>>,
}
...
impl<'session> EnvironmentFrame<'session> {
    pub fn new(parent: Option<Arc<EnvironmentFrame<'session>>>) -> EnvironmentFrame<'session> {
        EnvironmentFrame {
            parent,
    ...
    pub fn define(&self, name: Symbol, value: RV<'session>) {
        self.map.write().unwrap().insert(name, value);
    }

    pub fn assign(&self, key: &str, key_sym: Symbol, value: RV<'session>) -> Result<bool, HaltReason> {
    ...
    pub fn assign_at(
        env: &'session Arc<EnvironmentFrame<'session>>,
        distance: usize,
        key: &str,
        key_sym: Symbol,
        value: RV<'session>,
    ) -> Result<bool, HaltReason<'session>> {
        if distance == 0 {
    ...
    pub fn read(&self, key: &str, key_sym: &Symbol) -> Result<RV<'session>, HaltReason<'session>> {
