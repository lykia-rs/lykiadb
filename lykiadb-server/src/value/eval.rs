...
#[inline(always)]
pub fn eval_binary<'exec>(left_eval: RV<'exec>, right_eval: RV<'exec>, operation: Operation) -> RV<'exec> {
    /*
        TODO(vck):
...
mod property_tests {
    ...
    fn rv_strategy() -> impl Strategy<Value = RV<'static>> {
        prop_oneof![
            Just(RV::Undefined),
    ...
    fn numeric_rv_strategy() -> impl Strategy<Value = RV<'static>> {
        prop_oneof![
            any::<bool>().prop_map(RV::Bool),
            any::<f64>()
                .prop_filter("finite numbers", |x| x.is_finite())
