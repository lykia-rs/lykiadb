...
use crate::{
    engine::interpreter::{HaltReason, InterpretError, Interpreter}, util::Shared, value::{datatype::Datatype, RV}
};
...
fn object_rec(inner: Shared<FxHashMap<String, RV>>) -> Result<Datatype, HaltReason> {
    ...
    for (key, value) in inner.read().unwrap().iter() {
        match value {
            ...
            RV::Datatype(rvdt) => { type_map.insert(key.clone(), rvdt.clone()); }
            _ => {}
        }
    }
...
pub fn nt_object_of(_interpreter: &mut Interpreter, args: &[RV]) -> Result<RV, HaltReason> {
    match &args[0] {
        RV::Object(inner) => {
            Ok(RV::Datatype(object_rec(inner.clone())?))
        },
        _ => Err(HaltReason::Error(InterpretError::Other {
