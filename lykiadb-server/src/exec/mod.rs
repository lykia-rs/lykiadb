...
mod tests {
    ...
    fn test_execute_node_subquery() {
        ...
        assert_eq!(rows[0].get(&symbol).unwrap().if_object().unwrap().get("numbers").unwrap(), RV::Num(1.0));
        assert_eq!(rows[1].get(&symbol).unwrap().if_object().unwrap().get("numbers").unwrap(), RV::Num(2.0));
    }
    ...
    fn test_execute_node_projection_expr() {
        ...
        let projection_node = Node::Projection {
            ...
            fields: vec![SqlProjection::Expr {
                expr: Box::new(create_literal_expr(Literal::Str(Arc::new("projected_value".to_string())))),
